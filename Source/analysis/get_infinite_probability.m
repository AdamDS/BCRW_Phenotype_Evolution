...
%         trace_x,trace_y,INFRAT);
      cluster_spans = get_cluster_spans(population,num_clusters,trace_cluster,...
        trace_x,trace_y);
%       for gen = 1:ngen, 
...
%           infinite(gen) = 0;
%         else, 
%           infinite(gen) = length(finite(cu:cv)<1);
%         end
%       end
%       P(i,r) = length(find(infinite~=0))./ngen;
      P(i,r) = length(find(cluster_spans>=INFRAT))./ngen;
      if INFRAT~=0.989, 
        strper = num2str(INFRAT);
        dec = find(strper=='.');
        strper = [strper((dec+1):end) '_'];
      else, strper = [];  end
      save([new_dir_name 'cluster_spans_' strper clus_name run_name],'cluster_spans');
    end
    end
    end
    end
...
xlabel('\mu');  ylabel('spatial percolation probability');
