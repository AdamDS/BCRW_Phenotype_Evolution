...
%
% NEEDS TO BE FIXED: 
% new: descendant_clusters = []; 
% old: descendant_clusters = zeros(ipop,size(find(num_descendant_clusters),2));
%
% -ADS
function [num_descendant_clusters,descendant_clusters] = build_descendant_clusters(...
  base_name,run,dir_name), 
global SIMOPTS;
this_function = 'build_descendant_clusters';
...
if (~mat_exist([new_dir_name 'num_descendant_clusters_' clus_name run_name]) && ...
   ~mat_exist([new_dir_name 'descendant_clusters_' clus_name run_name])) || ...
   SIMOPTS.write_over,  
[nd,go,error] = try_catch_load([new_dir_name 'num_descendants_' base_name run_name],1,1);
...
if go, [tc,go,error] = try_catch_load([new_dir_name 'trace_cluster_' clus_name run_name],1,1);
...
%   descendant_clusters = []; 
  descendant_clusters = zeros(size(find(num_descendant_clusters(:,end))));
%% debug variables
% population = [6 8 7 4];
...
%% algorithm start
  for indiv = 1:ipop, 
    ...
    if desc_clus, 
      ...
      descendant_clusters = [descendant_clusters; zeros(1,size(descendant_clusters,2))];
    end
  end
%% algorithm end      
  save([new_dir_name 'num_descendant_clusters_' clus_name run_name],'num_descendant_clusters');
  save([new_dir_name 'descendant_clusters_' clus_name run_name],'descendant_clusters');
end %num_clusters
end %trace_cluster
end %population
end %parents
end %num_descendants
end %num_descendant_clusters & write_over
end %funciton