...
if SIMOPTS.periodic(1), 
  ...
  babies(outside,1) = mod(babies(outside,1),size(land,1)+0.5);
  bump = find(babies(:,1) < 0.5);
  babies(bump,1) = babies(bump,1) +0.5;
end
if SIMOPTS.periodic(2), 
  outside = find(babies(:,2) > (size(land,2)+0.5) | babies(:,2) < 0.5);
  babies(outside,2) = mod(babies(outside,2),size(land,2)+0.5);
  bump = find(babies(:,2) < 0.5);
  babies(bump,2) = babies(bump,2) +0.5;
end
