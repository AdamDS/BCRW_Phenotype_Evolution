...
if numel(SIMOPTS.SIMS)>1, 
  go_lta = mat_exist([dir_name 'lifetimes_' base_name int2str(SIMOPTS.SIMS(1)) '_' ...
                                             int2str(SIMOPTS.SIMS(2))]);
else, 
  go_lta = mat_exist([dir_name 'lifetimes_' base_name int2str(SIMOPTS.SIMS(1))]);
end
go_ltb = mat_exist([dir_name 'lifetimes_' base_name(1:end-1)]);
if ((~go_lta || ~go_ltb) && ~SIMOPTS.only_lt) || SIMOPTS.write_over,  
...
for run = SIMOPTS.SIMS 
  ...
  new_dir_name = split_cd(dir_name,run,SIMOPTS.split,SIMOPTS.make_dir,SIMOPTS.do_cd);
