...
% Data creation options
  % Save options
  save_coords = 1; %save trace_x & trace_y? no = 0, yes = 1
  save_seeds = 1;
  save_parents = 1; %save parents? no = 0, yes = 1
  save_kills = 1; %save kills? no = 0, yes = 1
    save_rivalries = 1; %save rivalries? no = 0, yes = 1
  source = 'H:\Babies_Root\';
  for_external = 1; %output to Data if 0, to Data\external if 1, to both Data & Data\external if 2
  split = 10; %output to Data if 0, output to Data\#_#+split if nonzero
  ...
  do_simulations = 0; %generates population, trace_x, trace_y, trace_noise, trace_cluster_seed*
...
% Birth settings
  ...
  dmu = 0.01;  mutability = [0.250:dmu:0.550];%[0.38:dmu:0.52];%[0.26:dmu:0.45];%0.350:dmu:0.400];%:dmu:0.420];
...
basic_map_sizes = [12];%[[12]' [12]']; %X and Y lengths for the basic map size
linear = 0;
