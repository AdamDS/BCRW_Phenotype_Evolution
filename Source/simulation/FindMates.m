...
% This is updated to use global SIMOPTS.
% This is updated for periodic boundaries. No search region with periodicity.
...
try_limits = [ttl(ittl) fin fin]'*ones(1,2); %limits 
dc_limits =  sqrt(2)*[0 ttl(ittl) fin fin fin]'*ones(1,2); %double check limits
warp_d = size(land)/2;
if SIMOPTS.reproduction~=1, 
  if SIMOPTS.reproduction==0, 
    M = zeros(pop,1); %initialize the mate-'n-cluster variable (MNC) for this generation
    ...
    for i=1:pop %for each baby
      ...
      if ~SIMOPTS.periodic(1) && ~SIMOPTS.periodic(2), 
        while M(i)==0 && SN(i)==0 && ~double_checked, 
          j = j +1;
          if ~last, 
        ...
        if SIMOPTS.periodic(1), 
          adx = abs(babies(:,1)-babies(i,1)); %x distance from i to all others
          fixx = find(adx>warp_d(1)); %find those closer by wrap
          adx(fixx) = warp_d(1)-adx(fixx); %fix those closer by wrap
          dx2 = adx.^2;
        else, dx2 = (babies(:,1)-babies(i,1)).^2;  end
        if SIMOPTS.periodic(2), 
          ady = abs(babies(:,2)-babies(i,2)); %y distance from i to all others
          fixy = find(ady>warp_d(2)); %find those closer by wrap
          ady(fixy) = warp_d(2)-ady(fixy); %fix those closer by wrap
          dy2 = ady.^2;
        else, dy2 = (babies(:,2)-babies(i,2)).^2;  end
        distance = dx2 +dy2;
        sort_distance = sort(distance);
        M(i) = find(distance==sort_distance(2));
        M_dist(i) = sqrt(sort_distance(2));
        SN(i) = find(distance==sort_distance(3));
        SN_dist(i) = sqrt(sort_distance(3));
      end
