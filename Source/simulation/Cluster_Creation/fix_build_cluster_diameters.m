%% GPUPAR_build_cluster_diameters.m
%
...
% -ADS 3*10*12
function [cluster_diameters] = GPUPAR_build_cluster_diameters(base_name,run_name), 
this_script = 'GPUPAR_build_cluster_diameters';
fprintf([this_script '\n']);
global SIMOPTS;
cluster_diameters = [];
    if exist([make_data_name('GPAR_cluster_diameters',base_name,run_name,0)...
              '.mat'],'file')~=2, 
    go = 1;
...
%       tc_of_gen = gpuArray([]);
%       coords_of_gen = gpuArray([]);
%       relatives_of_gen = gpuArray([]);
%       diameters = gpuArray([]);
%       cluster = gpuArray([]);
%       these_orgs = gpuArray([]);
%       N = gpuArray([]);
%       these_coords = gpuArray([]);
%       these_seeds = gpuArray([]);
%       shortest_paths = gpuArray([]);
%       to_check = gpuArray([]);
%       checked = gpuArray([]);
%       to_check_i = gpuArray([]);
%       from_d = gpuArray([]);
...
%       x1 = gpuArray([]);
%       y1 = gpuArray([]);
%       challenge_d_r_t = gpuArray([]);
%       old_d_r_t = gpuArray([]);
%       d_r_t = gpuArray([]);
%       picked_t = gpuArray([]);
      %initialize characteristic length holders
      local_cd = zeros(max(num_clusters),ngen);
      cd = gpuArray(local_cd);
      parfor gen = 1:ngen, 
  ...
  %% last part of debugging
      ...
      cluster_diameters = gather(cd(cd~=0));
      save(['GPAR_cluster_diameters_' base_name run_name],'cluster_diameters');
    end
    end
    end
    end
