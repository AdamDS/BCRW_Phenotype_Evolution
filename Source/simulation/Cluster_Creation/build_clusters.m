...
function [num_clusters,trace_cluster,orgsnclusters] = build_clusters(base_name,run,dir_name), 
global SIMOPTS;
this_function = 'build_clusters';
...
if ~mat_exist([new_dir_name 'num_clusters_' clus_name run_name]) || ...
   ~mat_exist([new_dir_name 'trace_cluster_' clus_name run_name]) || ...
   ~mat_exist([new_dir_name 'orgsnclusters_' clus_name run_name]) || ...
   SIMOPTS.write_over,  

  [tcs,go] = try_catch_load([new_dir_name 'trace_cluster_seed_' clus_name run_name],1,1);
  if go,  [p,go] = try_catch_load([new_dir_name 'population_' base_name run_name],1,1); %load population
  if go,  
  ...
  for gen = 1:NGEN, %for each generation
    ...
    for i=1:population(gen), %for each indiv of the population
      if tc(i)==0, %if indiv i is not labeled
        ...
        while notdoneyet, %while there are still relatives not labeled
%   asdf = debug_round(asdf);
  ...
  % else, 
  save([new_dir_name 'orgsnclusters_' clus_name run_name],'orgsnclusters');
  save([new_dir_name 'num_clusters_' clus_name run_name],'num_clusters');
  save([new_dir_name 'trace_cluster_' clus_name run_name],'trace_cluster');
  % end
  end %population
